...
class Compare_Widget(QtWidgets.QWidget):
    ...
    def _get_image_area(self, img_path):
        ...
        lbl_path = QtWidgets.QTextEdit()
        doc_path = QtGui.QTextDocument(img_path) #QtWidgets.QLabel(text=img_path)
        ...
        opt = doc_path.defaultTextOption()
        opt.setWrapMode(QtGui.QTextOption.WrapAtWordBoundaryOrAnywhere)
        doc_path.setDefaultTextOption(opt)
        lbl_path.setDocument(doc_path)
        ...
        bottom_line_hlayout.addWidget(like_checkbox)  # , 0, QtCore.Qt.AlignHCenter)
